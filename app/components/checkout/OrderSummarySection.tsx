...
export default function OrderSummarySection({
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-sm sticky top-20">
      ...
      <div className="text-xs text-gray-500 mb-4">{t('pricesIncludeVat')}</div>
      ...
      <div className="border-t pt-4 space-y-2">
        ...
        <div className="flex justify-between text-sm">
          <span className="text-gray-600">{t('subtotalInclVat')}</span>
        ...
        <div className="flex justify-between text-xs text-gray-500">
          <span>{t('vat', { percent: PRODUCT_VAT_PERCENT })}</span>
        ...
        {discountAmount > 0 && (
          <>
            <div className="flex justify-between text-sm text-green-700">
              <span>
                {couponType === 'manual'
                  ? (manualDiscountLabel || t('discount'))
                  : appliedCoupon
                    ? t('discountWithCode', { code: appliedCoupon })
                    : t('discount')}
              </span>
              <span className="font-medium">- {discountAmount.toFixed(2)} €</span>
            </div>
            <div className="flex justify-between text-xs text-gray-500">
              <span>{t('newSubtotalAfterDiscount')}</span>
              <span>{subtotalAfterDiscount.toFixed(2)} €</span>
            </div>
            <div className="flex justify-between text-xs text-gray-500">
              <span>{t('baseAfterDiscount')}</span>
              <span>{netSubtotal.toFixed(2)} €</span>
            </div>
            <div className="flex justify-between text-xs text-gray-500">
              <span>{t('vatAfterDiscount', { percent: PRODUCT_VAT_PERCENT })}</span>
        ...
        {shippingCostBase !== null && shippingCostBase > 0 && (
          <>
            ...
            <div className="flex justify-between text-xs text-gray-500">
              <span>{t('shippingVat', { percent: SHIPPING_VAT_PERCENT })}</span>
      ...
      <form onSubmit={onSubmit} className="mt-6">
        ...
        >
          {isLoading ? (
            ...
            <span>{t('salesSuspended')}</span>
          ) : (
            <span>{t('completeOrder', { total: total.toFixed(2) })}</span>
          )}
        </button>
      </form>
