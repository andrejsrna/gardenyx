      ...
      <div className="rounded-3xl border border-slate-800 bg-slate-900/70 p-6 shadow-xl shadow-emerald-900/10">
        ...
        {query.saved ? (
          <p className="mt-4 rounded-2xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-3 text-sm text-emerald-200">
            Produkt bol ulozeny a storefront bol revalidovany.
          </p>
        ) : null}
      </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,2fr)_minmax(320px,1fr)]">
          <div className="space-y-6">
            <section className="rounded-3xl border border-slate-800 bg-slate-900/70 p-6 shadow-xl shadow-emerald-900/10">
              <h2 className="text-lg font-semibold text-white">Slovencina</h2>
              <div className="mt-5 grid gap-4 md:grid-cols-2">
                <label className="space-y-2 md:col-span-2">
                  <span className="text-sm font-medium text-slate-200">Nazov</span>
                  <input
                    name="name"
                    defaultValue={product.name}
                    className="w-full rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3 text-sm text-white outline-none ring-0 transition placeholder:text-slate-500 focus:border-emerald-500"
                  />
                </label>
                <label className="space-y-2">
                  ...
                  <input
                    ...
                    className="w-full rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3 text-sm text-white focus:border-emerald-500"
                  />
                </label>
                <label className="space-y-2">
                ...
                <label className="space-y-2">
                  <span className="text-sm font-medium text-slate-200">Akciova cena</span>
                  <input
                    name="salePrice"
                    type="number"
                    step="0.01"
                ...
                <label className="space-y-2 md:col-span-2">
                  ...
                  <textarea
                    ...
                    className="w-full rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3 font-mono text-sm text-white focus:border-emerald-500"
                  />
                </label>
              </div>
            </section>
              ...
              <div className="mt-5 grid gap-4">
                <label className="space-y-2">
                  ...
                  <input
                    ...
                    defaultValue={typeof en.name === 'string' ? en.name : ''}
                    className="w-full rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3 text-sm text-white focus:border-emerald-500"
                  />
                </label>
                <label className="space-y-2">
                  ...
                  <textarea
                    ...
                    rows={3}
                    defaultValue={typeof en.shortDescription === 'string' ? en.shortDescription : ''}
                    className="w-full rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3 text-sm text-white focus:border-emerald-500"
                  />
                </label>
                <label className="space-y-2">
                  ...
                  <textarea
                    ...
                    rows={12}
                    defaultValue={typeof en.description === 'string' ? en.description : ''}
                    className="w-full rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3 font-mono text-sm text-white focus:border-emerald-500"
                  />
                </label>
              </div>
            ...
            <section className="rounded-3xl border border-slate-800 bg-slate-900/70 p-6 shadow-xl shadow-emerald-900/10">
              ...
              <div className="mt-5 grid gap-4">
                <label className="space-y-2">
                  <span className="text-sm font-medium text-slate-200">Nev</span>
                  <input
                    name="translations.hu.name"
                    defaultValue={typeof hu.name === 'string' ? hu.name : ''}
                    className="w-full rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3 text-sm text-white focus:border-emerald-500"
                  />
                </label>
                <label className="space-y-2">
                  <span className="text-sm font-medium text-slate-200">Rovid leiras</span>
                  <textarea
                    name="translations.hu.shortDescription"
                    rows={3}
                    defaultValue={typeof hu.shortDescription === 'string' ? hu.shortDescription : ''}
                    className="w-full rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3 text-sm text-white focus:border-emerald-500"
                  />
                </label>
                <label className="space-y-2">
                  <span className="text-sm font-medium text-slate-200">Leiras markdown</span>
                  <textarea
                    name="translations.hu.description"
                    rows={12}
                    defaultValue={typeof hu.description === 'string' ? hu.description : ''}
                    className="w-full rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3 font-mono text-sm text-white focus:border-emerald-500"
                  />
                </label>
              </div>
          ...
          <div className="space-y-6">
            <section className="rounded-3xl border border-slate-800 bg-slate-900/70 p-6 shadow-xl shadow-emerald-900/10">
              ...
              <div className="mt-5 grid gap-4">
                ...
                <label className="space-y-2">
                  <span className="text-sm font-medium text-slate-200">Weight</span>
                  <input
                    name="weight"
                    type="number"
                    step="0.001"
            ...
            <section className="rounded-3xl border border-slate-800 bg-slate-900/70 p-6 shadow-xl shadow-emerald-900/10">
              <h2 className="text-lg font-semibold text-white">Kategoria</h2>
              <div className="mt-4 space-y-3">
                {CATEGORY_OPTIONS.map((category) => (
                  <label key={category.slug} className="flex items-start gap-3 rounded-2xl border border-slate-800 px-4 py-3 text-sm text-slate-200">
                    <input
                      type="checkbox"
                      name="categories"
                      value={`${category.id}|${category.slug}|${category.name}`}
                      defaultChecked={categorySlugs.has(category.slug)}
            ...
            <section className="rounded-3xl border border-slate-800 bg-slate-900/70 p-6 shadow-xl shadow-emerald-900/10">
              <h2 className="text-lg font-semibold text-white">Hlavny obrazok</h2>
              <div className="mt-5 grid gap-4">
                <label className="space-y-2">
                  <span className="text-sm font-medium text-slate-200">Src</span>
                  <input
                    name="primaryImageSrc"
                    defaultValue={typeof primaryImage.src === 'string' ? primaryImage.src : ''}
                    className="w-full rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3 text-sm text-white focus:border-emerald-500"
                  />
                </label>
                <label className="space-y-2">
                  ...
                  <input
                    ...
                    defaultValue={typeof primaryImage.alt === 'string' ? primaryImage.alt : ''}
                    className="w-full rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3 text-sm text-white focus:border-emerald-500"
                  />
                </label>
              </div>
            ...
            <section className="rounded-3xl border border-slate-800 bg-slate-900/70 p-6 shadow-xl shadow-emerald-900/10">
              <SubmitButton
                idleLabel="Ulozit produkt"
                pendingLabel="Ukladam..."
