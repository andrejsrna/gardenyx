...
export async function sendReviewThankYouEmail(params: ReviewThankYouParams) {
  ...
  const content = `
    ...
    ${emailButton({ label: 'Nakúpiť s kupónom', url: (process.env.NEXT_PUBLIC_SITE_URL || 'https://najsilnejsiaklbovavyziva.sk') + '?coupon=' + encodeURIComponent(params.couponCode) })}
